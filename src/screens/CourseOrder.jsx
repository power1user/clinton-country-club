    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px 100px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <p style={{ fontFamily: '"Lora",serif', fontStyle: 'italic', fontSize: 12, color: G.muted, margin: '0 0 10px' }}>So the kitchen knows roughly when you'll be back at the clubhouse.</p>
        ...
        <div style={{ padding: '12px 14px', background: G.card, borderRadius: 4, border: `1px solid ${G.border}`, marginBottom: 20, display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <p style={{ fontFamily: '"Lora",serif', fontSize: 12, color: G.muted, margin: 0 }}>The kitchen will have your order <strong style={{ color: G.text }}>ready when you walk in</strong> from your round.</p>
        </div>
      </div>
      ...
      <div style={{ padding: '12px 20px 32px', background: G.bg, borderTop: `1px solid ${G.border}`, flexShrink: 0 }}>
        ...
        >
          <span style={{ fontFamily: '"Lora",serif', fontSize: 14, color: cart.length > 0 && hole && !busy ? '#F2EDE0' : G.muted, fontWeight: 500 }}>
            {busy ? 'Placing order…' : !hole ? 'Select your current hole to continue' : `Place Order · $${cartTotal}`}
          </span>
        </div>
